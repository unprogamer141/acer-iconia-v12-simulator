...
function applyTheme() {
  if (!darkMode) {
    document.getElementById('homescreen').style.background = 'linear-gradient(135deg, #e0eafc, #cfdef3)';
  } else {
    document.getElementById('homescreen').style.background = localStorage.getItem('wallpaper') || 'linear-gradient(135deg, #0f2027, #203a43, #2c5364)';
  }
...
function toggleDarkMode() {
  darkMode = !darkMode;
  applyTheme();
  alert(darkMode ? '🌙 Dark Mode On' : '☀️ Light Mode On');
}

function takeScreenshot() {
  const screen = document.getElementById('screen');
  const flash = document.createElement('div');
  flash.style.cssText = 'position:fixed;top:0;left:0;width:100%;height:100%;background:white;z-index:9999;opacity:1;transition:opacity 0.3s;pointer-events:none;';
  document.body.appendChild(flash);
  setTimeout(() => { flash.style.opacity = '0'; setTimeout(() => { document.body.removeChild(flash); }, 300); }, 100);
  alert('📸 Screenshot taken! In a real device it would be saved to Gallery.');
}
...
const contactsHTML =
  ...
  '</div><div id="contacts-list"></div>';

let dialValue = '';
function dialPress(val) {
  if (val === 'DEL') { dialValue = dialValue.slice(0, -1); }
  else if (val === 'CALL') { if (dialValue) { alert('📞 Calling ' + dialValue + '...'); dialValue = ''; } }
  else { dialValue += val; }
  const display = document.getElementById('dial-display');
  if (display) display.textContent = dialValue || 'Enter number';
}
function dialNumber(num) { dialValue = num; openApp('Dialer'); }
...
function buildCalendar(date) {
  ...
  let html = '<div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:15px;"><button onclick="changeMonth(-1)" style="background:rgba(255,255,255,0.1);border:none;color:white;padding:8px 14px;border-radius:10px;cursor:pointer;font-size:18px;">◀</button><div style="font-size:18px;font-weight:500;">' + months[month] + ' ' + year + '</div><button onclick="changeMonth(1)" style="background:rgba(255,255,255,0.1);border:none;color:white;padding:8px 14px;border-radius:10px;cursor:pointer;font-size:18px;">▶</button></div>';
...
let isDrawing = false, drawCtx = null, lastX = 0, lastY = 0, drawColor = '#ffffff', drawSize = 4, drawMode = 'pen';
function initDrawing() {
  ...
  canvas.addEventListener('touchstart', (e) => { e.preventDefault(); isDrawing = true; const rect = canvas.getBoundingClientRect(); lastX = e.touches[0].clientX - rect.left; lastY = e.touches[0].clientY - rect.top; });
  canvas.addEventListener('touchmove', (e) => { e.preventDefault(); if (!isDrawing) return; const rect = canvas.getBoundingClientRect(); const x = e.touches[0].clientX - rect.left; const y = e.touches[0].clientY - rect.top; drawCtx.beginPath(); drawCtx.moveTo(lastX, lastY); drawCtx.lineTo(x, y); drawCtx.strokeStyle = drawMode === 'eraser' ? '#1a1a2e' : drawColor; drawCtx.lineWidth = drawMode === 'eraser' ? 30 : drawSize; drawCtx.lineCap = 'round'; drawCtx.stroke(); lastX = x; lastY = y; });
  ...
  canvas.addEventListener('mousemove', (e) => { if (!isDrawing) return; const rect = canvas.getBoundingClientRect(); const x = e.clientX - rect.left; const y = e.clientY - rect.top; drawCtx.beginPath(); drawCtx.moveTo(lastX, lastY); drawCtx.lineTo(x, y); drawCtx.strokeStyle = drawMode === 'eraser' ? '#1a1a2e' : drawColor; drawCtx.lineWidth = drawMode === 'eraser' ? 30 : drawSize; drawCtx.lineCap = 'round'; drawCtx.stroke(); lastX = x; lastY = y; });
  canvas.addEventListener('mouseup', () => { isDrawing = false; });
}
function setDrawColor(color) { drawColor = color; drawMode = 'pen'; }
function setDrawMode(mode) { drawMode = mode; }
function clearCanvas() { if (!drawCtx) return; const canvas = document.getElementById('draw-canvas'); drawCtx.fillStyle = '#1a1a2e'; drawCtx.fillRect(0, 0, canvas.width, canvas.height); }

const drawingHTML =
  '<div style="display:flex;gap:8px;margin-bottom:10px;flex-wrap:wrap;">' +
  '<button onclick="setDrawMode(\'pen\')" style="padding:8px 12px;background:#1a73e8;border:none;color:white;border-radius:8px;cursor:pointer;">✏️ Pen</button>' +
  '<button onclick="setDrawMode(\'eraser\')" style="padding:8px 12px;background:#555;border:none;color:white;border-radius:8px;cursor:pointer;">⬜ Eraser</button>' +
  '<button onclick="clearCanvas()" style="padding:8px 12px;background:#e74c3c;border:none;color:white;border-radius:8px;cursor:pointer;">🗑️ Clear</button>' +
  '</div>' +
...
let battery = 100, charging = false;
...
function getAppIcon(name) {
  const icons = { 'Camera': '📷', 'Gallery': '🖼️', 'Settings': '⚙️', 'Files': '📁', 'Browser': '🌐', 'Calculator': '🧮', 'Clock': '🕐', 'Music': '🎵', 'Notes': '📝', 'Maps': '🗺️', 'About': '📱', 'Wallpaper': '🎨', 'Contacts': '👥', 'Calendar': '📅', 'Dialer': '📞', 'Drawing': '🎨' };
  return icons[name] || '📱';
}
...
function calcPress(val) {
  ...
  if (val === '=') { try { calcValue = String(eval(calcValue)); } catch(e) { calcValue = 'Error'; } }
...
function loadNotes() {
  ...
  if (!list) return;
  list.innerHTML = notes.reverse().map(n => '<div class="note-item"><div class="note-text">' + n.text + '</div><div class="note-time">' + n.time + '</div></div>').join('');
}
...
function stopwatchReset() { stopwatchStop(); stopwatchTime = 0; const el = document.getElementById('stopwatch-display'); if (el) el.textContent = '00:00'; }
...
function showFolder(name) {
  const folders = { 'Downloads': ['video1.mp4','photo1.jpg','document.pdf','music1.mp3'], 'Pictures': ['selfie.jpg','wallpaper.png','screenshot.jpg'], 'Music': ['song1.mp3','song2.mp3','playlist.m3u'], 'Videos': ['movie.mp4','clip1.mp4'], 'Documents': ['notes.txt','report.pdf','spreadsheet.xlsx'], 'DCIM': ['IMG_001.jpg','IMG_002.jpg','IMG_003.jpg'] };
  const files = folders[name] || [];
  const el = document.getElementById('files-content');
  if (!el) return;
  el.innerHTML = '<div class="file-header" onclick="showFilesHome()">◀ ' + name + '</div>' + files.map(f => '<div class="file-item">📄 ' + f + '</div>').join('');
}
function showFilesHome() { const el = document.getElementById('files-content'); if (!el) return; el.innerHTML = filesHomeHTML; }
...
function musicPause() { musicPlaying = false; clearInterval(musicInterval); document.getElementById('music-play-btn').textContent = '▶'; }
function musicToggle() { if (musicPlaying) musicPause(); else musicPlay(); }
function musicNext() { currentSong = (currentSong + 1) % songs.length; musicProgress = 0; updateMusicUI(); if (musicPlaying) { clearInterval(musicInterval); musicPlay(); } }
function musicPrev() { currentSong = (currentSong - 1 + songs.length) % songs.length; musicProgress = 0; updateMusicUI(); if (musicPlaying) { clearInterval(musicInterval); musicPlay(); } }
...
function loadGallery() {
  ...
  if (!grid) return;
  if (photos.length === 0) { grid.innerHTML = '<div class="app-placeholder">🖼️ No Photos Yet<br><br>Take a photo with the Camera app!</div>'; return; }
  grid.innerHTML = photos.reverse().map((p, i) => '<div class="gallery-item"><img src="' + p.src + '" style="width:100%;height:100%;object-fit:cover;border-radius:10px;" onclick="viewPhoto(' + i + ')"/><div class="gallery-time">' + p.time + '</div></div>').join('');
}
...
function closePhotoViewer() { const viewer = document.getElementById('photo-viewer'); if (viewer) viewer.style.display = 'none'; }
...
const aboutHTML = '<div style="text-align:center;padding:15px 0;margin-bottom:10px;"><div style="font-size:50px;font-weight:bold;color:#83b735;letter-spacing:4px;">Acer</div><div style="font-size:18px;margin-top:5px;">Iconia V12</div></div><div class="setting-item">📱 Model <span>Acer Iconia V12</span></div><div class="setting-item">🖥️ Display <span>11.97" IPS 2000x1200 90Hz</span></div><div class="setting-item">⚡ Processor <span>MediaTek Helio G99</span></div><div class="setting-item">🧠 RAM <span>8GB DDR4</span></div><div class="setting-item">💾 Storage <span>256GB UFS</span></div><div class="setting-item">📷 Rear Camera <span>8MP</span></div><div class="setting-item">🤳 Front Camera <span>5MP</span></div><div class="setting-item">🔋 Battery <span>8000mAh</span></div><div class="setting-item">⚡ Charging <span>USB-C Power Delivery 2.0</span></div><div class="setting-item">📶 WiFi <span>WiFi 6 (802.11ax)</span></div><div class="setting-item">🔵 Bluetooth <span>5.2</span></div><div class="setting-item">🎨 Color <span>Mist Green</span></div><div class="setting-item">🤖 Android <span>Android 15</span></div><div class="setting-item">🔊 Audio <span>Dual Stereo Speakers</span></div><div class="setting-item">💻 Build <span>Aluminum Chassis</span></div>';

const settingsHTML = '<div class="setting-item">📶 WiFi <span>Connected</span></div><div class="setting-item">🔵 Bluetooth <span>On</span></div><div class="setting-item">🔆 Brightness <span>80%</span></div><div class="setting-item">🔊 Volume <span>60%</span></div><div class="setting-item">🌐 Mobile Data <span>On</span></div><div class="setting-item">📍 Location <span>On</span></div><div class="setting-item">🔋 Battery <span>' + Math.round(battery) + '%</span></div><div class="setting-item">💾 Storage <span>256GB</span></div><div class="setting-item" onclick="toggleDarkMode()" style="cursor:pointer;">🌙 Dark/Light Mode <span>Toggle ›</span></div><div class="setting-item" onclick="takeScreenshot()" style="cursor:pointer;">📸 Screenshot <span>Take ›</span></div><div class="setting-item" onclick="openApp(\'Wallpaper\')" style="cursor:pointer;">🎨 Wallpaper <span>Change ›</span></div><div class="setting-item" onclick="openApp(\'About\')" style="cursor:pointer;">📱 About Device <span>Acer Iconia V12 ›</span></div>';
...
function openApp(appName) {
  ...
  addRecent(appName);
  const apps = {
    'Settings': settingsHTML, 'About': aboutHTML, 'Wallpaper': wallpaperHTML,
    'Camera': cameraHTML, 'Gallery': galleryHTML,
    'Files': '<div id="files-content">' + filesHomeHTML + '</div>',
    'Browser': browserHTML, 'Calculator': calcHTML, 'Clock': clockHTML,
    'Music': musicHTML, 'Notes': notesHTML, 'Maps': mapsHTML,
    'Battery': batteryHTML, 'Contacts': contactsHTML, 'Calendar': calendarHTML,
    'Dialer': dialerHTML, 'Drawing': drawingHTML,
  };
  if (apps[appName]) {
